...
function renderJobs(a){var b=document.getElementById('writingJobs');b.innerHTML=a.length?a.slice().reverse().slice(0,5).map(function(j){var ready=j.status==='draft_ready';return '<div class="job-card"><h4>Chapter '+Number(j.chapter_id||0)+' draft <span class="badge '+(ready?'good':'warn')+'">'+esc(String(j.status||'').replaceAll('_',' '))+'</span></h4><div class="tiny muted">'+Number(j.target_words||0).toLocaleString()+' words · '+esc(j.style_source||'')+(j.generated_word_count?' · generated '+Number(j.generated_word_count).toLocaleString()+' words':'')+'</div><p class="small muted">'+esc(j.message||'')+'</p>'+(ready?literaryJobHtml(j):'')+(ready&&j.generated_draft?'<details><summary class="btn small">Read generated draft</summary><div style="white-space:pre-wrap;font-family:Georgia,serif;font-size:13px;line-height:1.65;margin-top:10px;max-height:520px;overflow:auto">'+esc(j.generated_draft)+'</div></details>':'')+'</div>'}).join(''):'<div class="small muted">No chapter draft requested yet.</div>';b.querySelectorAll('.ba-learn').forEach(function(btn){btn.onclick=function(){sendLiteraryFeedback(btn.dataset.dim,btn.dataset.dir,btn)}})}
